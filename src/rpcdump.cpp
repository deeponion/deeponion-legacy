...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "importprivkey <DeepOnionprivkey> [label]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <DeepOnionaddress>\n"
            "Reveals the private key corresponding to <DeepOnionaddress>.");
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by DeepOnion %s (%s)\n", CLIENT_BUILD.c_str(), CLIENT_DATE.c_str());
...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid DeepOnion address or script");
    }
...

    {
        ...
        if (address.IsValid())
            ...
            //pwalletMain->SetAddressBookName(script.GetID(), strLabel);
