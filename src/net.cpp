...
{
    // Default 24-hour ban
    banTime = GetTime() + (!banTime) ? GetArg("-bantime", 60 * 60 * 24) : 0;
    {
        LOCK(cs_setBanned);
...
void StartNode(void *parg)
{
    ...
    else if (!NewThread(ThreadOnionSeed, NULL))
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_TORNET] > 0)
        printf("ThreadTorNet still running\n");
