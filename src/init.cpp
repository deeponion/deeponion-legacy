...
void Shutdown(void* parg)
{
    ...
    RenameThread("DeepOnion-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("DeepOnion version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  DeepOniond [options]                     " + "\n" +
                  "  DeepOniond [options] <command> [params]  " + _("Send command to -server or DeepOniond") + "\n" +
                  "  DeepOniond [options] help                " + _("List commands") + "\n" +
                  "  DeepOniond [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "DeepOnion:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("DeepOnion"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("DeepOnion"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: DeepOnion.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: DeepOniond.pid)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 18580 or testnet: 28580)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  DeepOnion is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "DeepOnion server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("DeepOnion"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of DeepOnion") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart DeepOnion to complete") << "\n";
